...
from backend.utils.upstreams import get_breaker
...
async def handle_get_schedule(request: web.Request) -> web.Response:
    ...
    # SSRF protection: only allow known schedule domains
    if url:
        from urllib.parse import urlparse as _urlparse
        _parsed_url = _urlparse(url)
        _allowed_hosts = {"english.mirea.ru", "mirea.ru", "www.mirea.ru", "mirea.ninja", "www.mirea.ninja", "schedule.mirea.ninja"}
        _host = (_parsed_url.hostname or "").lower()
        if _parsed_url.scheme not in ("https",) or _host not in _allowed_hosts:
            return web.json_response({"success": False, "message": "Недопустимый URL расписания"}, status=400)
...
async def handle_get_pulse_schedule(request: web.Request) -> web.Response:
    ...
    try:
        ...
        result = await svc.get_schedule(days=days)
        await svc.close()
        ...
        events = []
        for lesson in (result.lessons or []):
            # Build description with type + teacher + subgroup
            desc_parts = []
